            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    crop_obj = im0[int(xyxy[1]):int(xyxy[3]),int(xyxy[0]):int(xyxy[2])]
                     # Check if the cropped object is valid
                    if crop_obj.size > 0 and crop_obj.shape[0] > 0 and crop_obj.shape[1] > 0:
                        blur = cv2.blur(crop_obj, (blurratio, blurratio))
                        im0[int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])] = blur
                    else:
                        print(f"Warning: Invalid cropping dimensions: x1={int(xyxy[0])}, y1={int(xyxy[1])}, x2={int(xyxy[2])}, y2={int(xyxy[3])}")
                    #..................................................................
...
def worker(source_chunk):
    detect(source_chunk)

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    all_sources = [parser.add_argument('--weights', nargs='+', type=str, default='yolov7.pt', help='model.pt path(s)'),
    ...
    num_processes = 4  # Or however many you want
    chunk_size = len(all_sources) // num_processes

    source_chunks = [all_sources[i:i + chunk_size] for i in range(0, len(all_sources), chunk_size)]
    with Pool(num_processes) as p:
        p.map(worker, source_chunks)
